...
// Written by Micha Ober @ http://www.hardwareluxx.de
//
...
namespace SmartDataApp
{
    ...
    {
        public static Dictionary<int, string> GetDisks(out int numDisks)
        {
            numDisks = 0;
            var diskDict = new Dictionary<int, string>();
            try
            {
                var disks = new ManagementObjectSearcher("SELECT * FROM Win32_DiskDrive");
                foreach (ManagementObject queryObj in disks.Get())
                {
                    Console.Write("[" + numDisks + "] ");
                    Console.WriteLine(queryObj.GetPropertyValue("Model"));
                    diskDict.Add(numDisks, queryObj.GetPropertyValue("PNPDeviceID").ToString());
                    numDisks++;
                }
            }
        ...
        public static int SelectDiskId(int numDisks)
        {
            int diskId = 0;
            bool valid = false;
            ...
            while (!valid)
            {
                Console.Write("Enter disk number to monitor: ");
                valid = int.TryParse(Console.ReadLine(), out diskId) &&
                    (diskId >= 0 && diskId < numDisks);
                if (!valid)
        ...
        public static string SelectFileName()
        {
            ...
            while (!valid)
            {
                ...
                valid = (!string.IsNullOrEmpty(fileName) &&
                ...
                if (valid)
                {
                    ...
                    valid = !File.Exists(Path.Combine(cwd, fileName));
                }                    

                if (!valid)
                    Console.WriteLine("Invalid filename or file already exists");
            }

            fileName = Path.Combine(cwd, fileName);
            return fileName;
        }

        public static Dictionary<string, int> getDiskInfo(string pnpId)
        {
            var result = new Dictionary<string, int>();

            try
            {
                var searcher = new ManagementObjectSearcher("root\\WMI", "SELECT * FROM MSStorageDriver_ATAPISmartData  WHERE InstanceName='" + pnpId + "'");
                ...
                foreach (ManagementObject queryObj in searcher.Get())
                {
                    ...
                    {
                        byte[] data = { b.VendorData[0], b.VendorData[1] }; // Use only two bytes
        ...
        public static void Main()
        {
            ...
            int diskId = SelectDiskId(numDisks);
            string pnpId = diskDict[diskId];
            pnpId = pnpId.Replace("\\", "\\\\") + "_0";

            string fileName = SelectFileName();
            Console.WriteLine("Log: " + fileName);
            Console.WriteLine("Stop logging by pressing ESC.");

            int n = 0;
            const string sep = ","; // Separator used in the output file
            ...
            using (StreamWriter file = new StreamWriter(fileName))
            {
                while (true)
                {
                    ...
                    var data = getDiskInfo(pnpId);

                    if (n == 0) // Write header
                        file.WriteLine("Timestamp" + sep + string.Join(sep, data.Keys));

                    file.Write(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss:fff") + sep); // Iometer format
                    file.WriteLine(string.Join(sep, data.Values));

                    Thread.Sleep(2000); // Time between two measurements
                    n++;
                }
            }
